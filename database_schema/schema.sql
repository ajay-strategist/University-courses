...
-- -------------------------------------------------------------------------------------
-- 14. PROFILE TO AUTH SYNC TRIGGER
-- -------------------------------------------------------------------------------------

-- Create trigger function to automatically sync uct_profiles to auth.users
CREATE OR REPLACE FUNCTION public.uct_tg_sync_profile_to_auth()
RETURNS TRIGGER AS $$
BEGIN
  -- 1) Check if the user ID already exists in auth.users
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = NEW.id) THEN
    -- 2) Check if email is already taken in auth.users (to avoid duplicate email key violation)
